...
from src.models import FinalDocument
...
class DocumentRenderer:
    ...
    def render_markdown(self, document: FinalDocument, output_path: Path) -> Path:
        template = self.env.get_template("final_document.md.j2")
        content = template.render(
            metadata=document.metadata,
            domain_model=document.domain_model,
            conversation_summary=document.conversation_summary,
            interaction_type=document.interaction_type,
            personas=document.personas,
            srs_content=document.srs_content or "*SRS não disponível.*",
        )
